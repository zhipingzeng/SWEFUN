...
resolution_target_3=imread('picture\target-1-adjust.tif');  %change with 1 2 5 6 or 1 2 3 4
% resolution_target_3=zeros(500,500);
[row, col] = find( resolution_target_3 >= 50);   % 200 row和col均为227*1的矩阵
num = size(row, 1);           %
bb=aa;
for i = 1:1:num                
...
%找到所有大于50的值所在位置，并 对其进行重新赋值为1          
%% Generate power law blinking
bright1 = app.bright1;
darkness = app.darkness;
n1_R=bright1/(1+Alpha1);  %change Reflection rates = 0.10,0.51,6.03,43.1
n1_T=bright1-n1_R;  
n2=darkness/(1+Alpha1);
n2_T=darkness-n2;
% n2=0;
% n2_T=0;
C = zeros(num,2*1000);
...
a=ones(1,num);
for frame=1:150
    for m=1:1:num   %c大于0为亮态，同时此点c-1，（减少后续帧亮态概率）。else为暗态，同时
       if C(m,a(m))>0
           aa(row(m), col(m))  = n1_R;
           tt(row(m), col(m))  = n1_T;
           C(m,a(m))=C(m,a(m))-1;
...
PSFXY=PSFX'*PSFX; 
jj=conv2(aa,PSFXY);  %image convolution R
kk=conv2(tt,PSFXY);  %image convolution T
           
imshow(uint16(jj)*app.strengthen,'Parent',app.blinkingf);pause(0.01) 
filepath=strcat(InputFilepath,'\RawData\1\R\');  %change filepath
mkdir(filepath);
imwrite(uint16(jj),[filepath int2str(frame) '.tif']);
filepath=strcat(InputFilepath,'\RawData\1\T\');  %change filepath
mkdir(filepath);
