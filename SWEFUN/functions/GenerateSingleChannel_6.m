...
resolution_target_3=imread('picture\target-6-adjust.tif');  %change with 1 2 5 6 or 1 2 3 4
...
n1_R=bright6/(1+Alpha6);  %change Reflection rates = 0.10,0.51,6.03,43.1
n1_T=bright6-n1_R;
n2=darkness/(1+Alpha6);
n2_T=darkness-n2;
% n2=0;
% n2_T=0;
C = zeros(num,2*1000);
...
filepath=strcat(InputFilepath,'\RawData\6\R\');  %change filepath
mkdir(filepath);
imwrite(uint16(jj),[filepath int2str(frame) '.tif']);
filepath=strcat(InputFilepath,'\RawData\6\T\');  %change filepath
mkdir(filepath);
